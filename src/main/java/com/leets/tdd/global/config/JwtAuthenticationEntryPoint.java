...
public class JwtAuthenticationEntryPoint implements AuthenticationEntryPoint {
    ...
    public void commence(
        ...
        if (("POST".equalsIgnoreCase(request.getMethod())
                && requestPath.matches("/api/v1/parties"))
                || requestPath.matches("/api/v1/parties/me")
                || ("DELETE".equalsIgnoreCase(request.getMethod())
                && requestPath.matches("/api/v1/parties/[^/]+"))
                || requestPath.matches("/api/v1/parties/[^/]+/join")
                || requestPath.matches("/api/v1/parties/[^/]+/participants")
                || requestPath.matches("/api/v1/parties/[^/]+/complete")
                || requestPath.matches("/api/v1/parties/[^/]+/order")
                || requestPath.matches("/api/v1/parties/[^/]+/close")) {
